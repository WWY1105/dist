...
<div id="missionInAfterEvaluteBuss">
    ...
    <div class="topBox bgW">
        ...
        <div class="hang flexSpace">
            <p class="left"><span class="title">服务提供者：</span><span v-id="nickname">{{nicknameArr.length==0?'':nicknameArr.join(',')}}</span></p>
    ...
    <div class="btnBox bgW"><button class="long_btn" @click.stop="giveRed">赞赏作者红包</button></div>
...
export default {
    ...
    methods: {
        ...
        getMissiondetail(id) {
            var that = this;
            that.$http('get', that.$store.state.baseUrl + 'api/Task/' + id + "?uid=" + that.$store.state.uid).then(function (res) {
                if (res.data.code == '00') {
